...
class MockDirectStreamEventReader implements IDirectStreamEventReader {
  ...
  private parentStreams: Map<string, DirectStream> = new Map();
  ...
  async getByParentAndEventSlug(parentSlug: string, eventSlug: string) {
    for (const event of this.events.values()) {
      const parent = this.parentStreams.get(event.directStreamId);
  ...
  // Test helpers
  _setEvent(event: DirectStreamEvent) {
    this.events.set(event.id, event);
  }
  
  _setParent(parent: DirectStream) {
    this.parentStreams.set(parent.id, parent);
  }
}
...
describe('DirectStreamEventService', () => {
  ...
  describe('deleteEvent', () => {
    it('should soft delete by default', async () => {
      ...
      const event = writer._getEvent(created.id);
