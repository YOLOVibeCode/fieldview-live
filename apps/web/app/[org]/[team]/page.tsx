...
export default function OrgTeamStreamPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/api/streams/${encodeURIComponent(org)}/${encodeURIComponent(team)}`)
      .then((res) => {
        if (res.status === 404) {
          setStatus('offline');
          return null;
  ...
  async function handleSaveUrl() {
    ...
    try {
      const res = await fetch(`${API_URL}/api/streams/${encodeURIComponent(org)}/${encodeURIComponent(team)}`, {
        method: 'POST',
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-4">
        <div className="w-full max-w-6xl">
          ...
          <div className="mt-4 text-center text-gray-500 text-xs">
            <p>
              Powered by FieldView.Live • Share this link: <strong>{`fieldview.live/${org}/${team}`}</strong>
            </p>
          </div>
        </div>
      </div>
