...
test('LIVE: admin console search uses automation-friendly selectors', async ({ page, request }) => {
  ...
  // Create admin and login (simplified - adjust based on your auth flow)
  // This assumes you have a way to get an admin session token
  // For now, we'll test the UI selectors assuming user is logged in
  ...
  await expect(page.getByText(/No.*matches|Viewers|Games|Purchases/i)).toBeVisible();
});
