...
export function DirectHlsAdminPage(props: {
  apiPrefix: '/api/tchs' | '/api/direct';
  streamKey: string;
  title: string;
  subtitle?: string;
  ...
  useEffect(() => {
    fetch(`${API_URL}${props.apiPrefix}/${encodeURIComponent(props.streamKey)}`)
      .then((res) => {
        if (res.status === 404) {
          setStatus('offline');
          return null;
  ...
  }, [props.apiPrefix, props.streamKey]);
  ...
  useEffect(() => {
    function onFullscreenChange() {
      ...
      setIsFullscreen(Boolean(doc.fullscreenElement ?? doc.webkitFullscreenElement ?? video?.webkitDisplayingFullscreen));
    }
  ...
  async function handleSaveUrl() {
    ...
    if (password.trim() !== props.adminPassword) {
    ...
    setMessage('Saving...');
    try {
      const res = await fetch(`${API_URL}${props.apiPrefix}/${encodeURIComponent(props.streamKey)}`, {
        method: 'POST',
      ...
      if (!res.ok) {
        ...
        setMessage(err?.error || 'Failed to save stream URL');
        return;
      }
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col" data-testid="container-direct-page">
      {/* Quick Edit Buttons */}
      {!isEditing && (
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-4">
        <div className="w-full max-w-6xl">
            ...
            {status === 'offline' && (
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="text-center text-white">
                  ...
                  <p className="text-sm text-gray-400 mb-4">The stream will start soon. Check back shortly.</p>
                  <button
                    onClick={() => setIsEditing(true)}
          ...
          <div className="mt-4 text-center text-gray-500 text-xs">
            <p data-testid="text-share">
              Powered by FieldView.Live • Share this link: <strong>{props.sharePath}</strong>
            </p>
          </div>
        </div>
      </div>
